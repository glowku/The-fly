...
from agent.memory import (
    load_coverage, save_coverage, load_quality, save_quality,
    log_event, compute_objective_score,
)
...
def run_cycle(dry_run: bool = False):
    ...
    print(f"[plan] {target['reason']} → « {title} » (action={action}, prio={target.get('priority')}, skill={target.get('skill')})", flush=True)
    ...
    register_processed(title, depth, sources=[summary["url"]] if summary.get("url") else [])
    ...
    ok = git_commit_push([f"wiki/{title}.md", "state/", "DASHBOARD.md"], msg)
